...
export const events = async (req, res) => {
	...
	// 								);
	// 								DateStart = DateStart.substring(DateStart.lastIndexOf(':') + 1, DateStart.length);
	// 								let DateEnd = fileData.substring(
	// 									fileData.lastIndexOf('DTEND;'),
	// 									fileData.lastIndexOf('CLASS:')
	// 								);
	// 								DateEnd = DateEnd.substring(DateEnd.lastIndexOf(':') + 1, DateEnd.length);
	// 								eventsDta[x] = {
	...
	emailFetchedData === 'Error' || emailFetchedData === undefined || emailFetchedData === 'undefined'
		...
		: emailFetchedData.map(async (data) => {
				if ((await data) !== undefined) {
					// console.log('end data:-----:---------', typeof data.invites, '--invites--', data.invites);

					// console.log(
					// 	'invite filter data:-----:---------',
					// 	typeof data.invites,
					// 	data.invites
					// 		.filter((invite) => invite !== data.organizer && invite !== 'myguardiansixtesting@gmail.com')
					// 		.join(',')
					// );
	...
	// 			dateStart:
	// 				item.DateStart instanceof Date && !isNaN(item.DateStart)
	// 					? dateFormat(item.DateStart, 'dddd, mmmm dS, yyyy, h:MM:ss TT')
	// 					: dateFormat(formatedDate(item.DateStart), 'dddd, mmmm dS, yyyy, h:MM:ss TT'),
	// 			dateEnd:
	// 				item.DateEnd instanceof Date && !isNaN(item.DateEnd)
	// 					? dateFormat(item.DateEnd, 'dddd, mmmm dS, yyyy, h:MM:ss TT')
	// 					: dateFormat(formatedDate(item.DateEnd), 'dddd, mmmm dS, yyyy, h:MM:ss TT'),
...
// 					headerObject.subject[0] = headerObject.subject[0].replace('(havea@goodmeeting.today)', '');
